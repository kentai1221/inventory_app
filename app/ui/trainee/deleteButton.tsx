...
import { deleteTrainee } from '@/app/lib/actions';
import { PencilIcon, PlusIcon, TrashIcon } from '@heroicons/react/24/outline';
import {
    HiOutlineExclamationCircle,
  } from "react-icons/hi";

export function DeleteTrainee({ id }: { id: string }) {
    //const { isOpen, open, close } = useModal();
    const [isOpen, setOpen] = useState(false);
    const deleteTraineeWithId = deleteTrainee.bind(null, id);
    ...
    return (
        <>
            ...
            <span className="sr-only">{i18n('del_trainee')}</span>
            ...
            <Modal onClose={() => setOpen(false)} show={isOpen} size="md">
                ...
                <div className="flex flex-col items-center gap-y-6 text-center">
                    ...
                    {i18n('del_trainee_msg')}
                    </p>
                    <div className="flex items-center gap-x-3">
                    <form action={deleteTraineeWithId} className="inline-block">
                    <Button
                        color="failure"
                        theme={{ base: "px-0" }}
                        type="submit"
